...
function Canvas() {
  ...
  // Función para eliminar imágenes seleccionadas al presionar la tecla "Suprimir"
  // eslint-disable-next-line react-hooks/exhaustive-deps
  const handleDeleteKeyPressed = (e) => {
    if (e.key === "Delete" || e.key === "Backspace") {
      const filteredImages = images.filter((_, i) => i !== selectedId);
      setImages(filteredImages);
      setSelectedId(null);
    }
  };

  // Agregar event listener para el evento de teclado
  useEffect(() => {
    window.addEventListener("keydown", handleDeleteKeyPressed);
    return () => {
      window.removeEventListener("keydown", handleDeleteKeyPressed);
    };
  }, [handleDeleteKeyPressed, images, selectedId]);
